...
*/
std::tuple<std::vector<Point3D>, std::vector<Point2D>, std::vector<Point3D>> readMeshObj(std::string path) {
	...
	while (std::getline(file, line)) {
		...
		if (line[0] == 'v' && line[1] == ' ') {
			Point3D vertex = Point3D(std::stod(content[1]), std::stod(content[2]), std::stod(content[3]));
...
std::tuple<std::vector<Point3D>, std::vector<Point2D>, std::vector<Point3D>> readMesh(std::string path, MeshType type) {
	switch (type)
	{
	case OBJ:
